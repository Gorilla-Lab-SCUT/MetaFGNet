...
def train(train_loader_source, train_loader_source_batch, train_loader_target, train_loader_target_batch, model_source, model_target, criterion, optimizer, epoch, args, meta_train_lr):
    ...
    if train_loader_source:
        ...
        except StopIteration:  ## the data iterator comes to the end.
    ...
    if not args.meta_sgd:
        ...
        if args.args.auxiliary_dataset == 'imagenet' and not args.pretrained:
    ...
    for param in model_target_temp.parameters():
        temp_grad.append(param.grad.data.clone())
    grad_for_task_mtrain.extend(temp_grad)  ## d(meta-loss) / d(\theta^t'), it can be viewed as the first-order approximation of the
    
    grad_of_meta_test = copy.deepcopy(grad_for_task_mtrain)
    
    if args.second_order_grad:  ## the gating whether to use the computational expensive second-order gradient.
        for m in range(args.num_updates_for_gradient):
            ...
            for k in range(args.num_updates_for_gradient - m):
                ...
                '''
                for i in range(1, len(grad_params) + 1):
                    grad_mul_weight_sum = 0
                    for j in range(len(grad_params) - i, len(grad_params)):
                        grad_mul_weight_sum += grad_params[j].sum()
                    ipdb.set_trace()
                    
                    try:
                        # grad_mul_weight_sum.backward(retain_graph=True)
                        grad2_params = torch.autograd.grad(grad_mul_weight_sum, model_target_temp.parameters(), create_graph=True)
                    except:
                        print(i)
        
                    grad_mul_weight_sum_clockwise = 0
                    for j in range(i):
                        grad_mul_weight_sum_clockwise += grad_params[j].sum()
                    ipdb.set_trace()
        
                    try:
                        grad_mul_weight_sum_clockwise.backward(retain_graph=True)
                        # grad2_params = torch.autograd.grad(grad_mul_weight_sum_clockwise, model_target_temp.parameters(), create_graph=True)
                    except:
                        print(i)
                '''
    ...
    if args.first_meta_update:  ## Also apply the first gradient for parameter update, We Do Not adopt this strategy.
        count = 0
    ...
    if train_loader_source:
        count = 0
        for param in model_source.parameters():  ## The gradient corresponding to the regularization loss.
            temp_grad = param.grad.data.clone()
            temp_grad.zero_()
